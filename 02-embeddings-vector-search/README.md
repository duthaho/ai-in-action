...
- **[app.py](app.py)** — Full FastAPI application with embedding, indexing, and RAG search
...
- **[test_embeddings.py](test_embeddings.py)** — Tests for all components
...
|---|---|---|
| `POST` | `/documents` | Add a document (auto-chunks and embeds) |
| `POST` | `/search` | Semantic search across all documents |
| `POST` | `/ask` | RAG — search + LLM answer generation |
